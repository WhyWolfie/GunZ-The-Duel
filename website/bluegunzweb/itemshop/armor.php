...
$queryq = "SELECT COUNT(*) as num FROM ItemShop WHERE Category='armor'";
...
$res = mssql_query("SELECT TOP $limit * FROM ItemShop WHERE ID not in (SELECT TOP
                    $start ID FROM ItemShop ORDER BY ID DESC) AND Category='armor' ORDER BY ID DESC");
...
<div class="itemshp">
 ...
 <?php }else{ ?>
 <a href="index.php?act=itemshop&amp;do=armor&amp;buy=<?php echo $item['ItemID']; ?>"><button class="itmshp_button"><?php echo $itmshop['9']; ?></button></a>
 <a href="index.php?act=itemshop&amp;do=armor&amp;gift=<?php echo $item['ItemID']; ?>"><button class="itmshp_button"><?php echo $itmshop['10']; ?></button></a>
 <?php } ?>
 </div>
 </td></tr></table>
 <?php } ?>
...
<?php
		...
		// Previous
		if ($page > 1){
			$paginate.= "<a href='index.php?act=itemshop&do=armor&amp;page=$prev'>Prev</a>";
		}else{
	    ...
	    for ($counter = 1; $counter <= $lastpage; $counter++){
	     if ($counter == $page){
	      ...
	      $paginate.= "<a href='index.php?act=itemshop&do=armor&amp;page=$counter'>$counter</a>";}					
	     }
		...
		$paginate.= "...";
		$paginate.= "<a href='index.php?act=itemshop&do=armor&amp;page=$LastPagem1'>$LastPagem1</a>";
		$paginate.= "<a href='index.php?act=itemshop&do=armor&amp;page=$lastpage'>$lastpage</a>";	
        // Middle hide some front and some back		
	    }elseif($lastpage - ($stages * 2) > $page && $page > ($stages * 2)){
	  	  $paginate.= "<a href='index.php?act=itemshop&do=armor&amp;page=1'>1</a>";
		  $paginate.= "<a href='index.php?act=itemshop&do=armor&amp;page=2'>2</a>";
		  $paginate.= "...";
	...
	// Next
	if ($page < $counter - 1){ 
		$paginate.= "<a href='index.php?act=itemshop&do=armor&amp;page=$next'>next</a>";
	}else{
